...
use klukai_client::CorrosionClient;
use klukai_types::{
    api::{ColumnType, Statement},
    config::ConsulConfig,
    consul::{self, AgentCheck, AgentService, Client},
};
...
pub async fn run<P: AsRef<Path>>(
    ...
    let consul = consul::Client::new(config.client.clone())?;
...
async fn setup(corrosion: &CorrosionClient) -> eyre::Result<()> {
    ...
    struct ColumnInfo {
        name: String,
        kind: klukai_types::api::ColumnType,
    }
...
async fn update_hashes(
    ...
    if !statements.is_empty()
        && let Some(e) = corrosion
            .execute(&statements, None)
            .await?
            .results
            .into_iter()
            .find_map(|res| match res {
                klukai_types::api::ExecResult::Execute { .. } => None,
                klukai_types::api::ExecResult::Error { error } => Some(error),
            })
    {
...
enum ExecuteError {
    #[error("client: {0}")]
    Client(#[from] klukai_client::Error),
    #[error("sqlite: {0}")]
    Sqlite(String),
}
...
mod tests {
    ...
    use klukai_types::{spawn::wait_for_all_pending_handles, tripwire::Tripwire};
