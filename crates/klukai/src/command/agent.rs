...
pub async fn run(
    ...
    let (agent, bookie, _, handles) =
        klukai_agent::agent::start_with_config(config.clone(), tripwire.clone())
            .await
    ...
    if !config.db.schema_paths.is_empty() {
        let client = klukai_client::CorrosionApiClient::new(*config.api.bind_addr.first().unwrap());
        match client
...

    {
        ...
        tokio::spawn(async move {
            for metrics in runtime_monitor.intervals() {
                ...
                gauge!("corro.tokio.global_queue_depth").set(metrics.global_queue_depth as f64);
