...
impl QueryHandle {
    async fn body(
        &mut self,
    ) -> Result<(SubscriptionStream<Vec<SqliteValue>>, bool), klukai_client::Error> {
...
impl Engine {
    pub fn new<W: Writer>(client: klukai_client::CorrosionApiClient) -> Self {
...
mod tests {
    ...
    async fn test_sql() {
        ...
        let client = klukai_client::CorrosionApiClient::new(ta.agent.api_addr());

        client
            .schema(&[Statement::Simple(klukai_tests::TEST_SCHEMA.into())])
            .await
            .unwrap();
